...
const Analise = () => {
  const faseEngenhariaConsequencias = useSelector((state: RootState) => state.faseEngenharia.faseEngenharia)
  const dispatch = useDispatch()

  useEffect(() => {
    dispatch(showLoading())
    fetch('http://localhost:8080/api/ontology/fase-engenharia/consequencia')
      .then((response) => {
        if (response.ok) {
  ...
  return (
    <div className={styles.container}>
      ...
      {faseEngenhariaConsequencias.map((consequencia) => {
        return (
